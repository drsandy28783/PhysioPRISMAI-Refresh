...
# Rate limit storage: {uid: [timestamp1, timestamp2, ...]}
...
def check_rate_limit(uid):
    ...
    # Clean old entries
    _rate_limit_store[uid] = [ts for ts in _rate_limit_store[uid] if ts > cutoff]

    # Check if under limit
    if len(_rate_limit_store[uid]) >= _rate_limit_max_requests:
        return False
...
def require_firebase_auth_with_rate_limit(f):
    """
    Same as require_firebase_auth but with rate limiting.
    Use this decorator instead if you want rate limiting enabled.
    """
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # First, authenticate
        ...
        try:
            ...
            # Check rate limit
            if not check_rate_limit(g.firebase_user['uid']):
                logger.warning(f"Rate limit exceeded for user: {g.firebase_user['uid']}")
