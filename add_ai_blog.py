"""
Add AI in Physiotherapy blog post to Cosmos DB
"""
...
def add_ai_blog_post():
    """Add the AI in Physiotherapy blog post to the database"""

    try:
        db = get_cosmos_db()

        # Create the blog post with internal links and CTAs
        blog_post = {
            'title': 'AI in Physiotherapy: How Clinical Decision Support Is Changing Practice',
            'slug': 'ai-in-physiotherapy-clinical-decision-support',
            'content': '''
# AI in Physiotherapy: How Clinical Decision Support Is Changing Practice

The physiotherapy profession has always been built on clinical reasoning — the ability to gather patient information, form hypotheses, and arrive at sound clinical decisions. But as caseloads grow and evidence bases expand, even the most experienced clinician can benefit from intelligent support at the point of care.
...
> **Related:** Learn more about [clinical reasoning frameworks](/blog/post/clinical-reasoning-framework) and how structured assessment improves decision-making.

## How AI Supports the Clinical Reasoning Process

Clinical reasoning in physiotherapy typically follows a structured pathway — from history taking and subjective examination through objective assessment, provisional diagnosis, and treatment planning. AI can add value at every stage of this process.
...
> **Learn more:** Read our guide on [comprehensive history taking in physiotherapy](/blog/post/history-taking-physiotherapy).
...
> **Related:** Explore our article on [evidence-based treatment planning](/blog/post/evidence-based-treatment-planning) for more insights.
...
Imagine completing a subjective examination and receiving a prompt noting that the combination of symptoms your patient described is commonly associated with central sensitisation — suggesting you consider specific screening tools. Or finishing your [objective assessment](/blog/post/objective-assessment-guide) and seeing a ranked list of differential diagnoses based on your clinical findings, with links to the supporting evidence.

These are not interruptions to your workflow. They're enhancements that integrate seamlessly into the [documentation process](/blog/post/digital-documentation-best-practices), appearing when relevant and staying out of the way when they're not.
...
<div style="background: #e8f5e9; padding: 24px; border-radius: 8px; margin: 32px 0; border-left: 4px solid #7cb342;">
  ...
  <a href="/register" style="display: inline-block; background: #7cb342; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: 600; margin-right: 12px;">Start Free Trial</a>
...
<div style="background: #fff3e0; padding: 24px; border-radius: 8px; margin: 32px 0; border-left: 4px solid #ff9800; text-align: center;">
  ...
  <a href="/register" style="display: inline-block; background: #ff9800; color: white; padding: 14px 32px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 1.1em;">Get Started Free →</a>
...
*Dr. Sandeep Rao is the founder of PhysiologicPRISM and creator of the PRISM Clinical Reasoning Framework. [Learn more about our platform](/pricing) or [contact us](mailto:support@physiologicprism.com) for a personalized demo.*
        ...
        print(f"\nView at: /blog/{doc_id}")
        print(f"Or at: /blog/post/{blog_post['slug']}")
...
if __name__ == '__main__':
    print("=" * 70)
    print("ADD AI IN PHYSIOTHERAPY BLOG POST")
    print("=" * 70)
    add_ai_blog_post()
