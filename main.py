...
# ---- AI: Objective Assessment (per-field) ----
...
def objective_assessment_field_suggest(field):
    ...
    prev = _collect_prev(data)  # expects {"previous": {...}} but tolerates {}
    subj_summary = "\n".join(f"- {k.replace('_',' ').title()}: {v}"
                             for k, v in prev.get("subjective", {}).items() if v)
    assess_summary = "\n".join(
        f"- {k.replace('_',' ').title()}: {(v.get('choice','') if isinstance(v, dict) else v)}"
        for k, v in prev.get("assessments", {}).items()
        if (v if not isinstance(v, dict) else (v.get('choice') or v.get('details')))
    )

    prompt = (
        "A physiotherapist is selecting options during an objective assessment (no identifiers).\n"
        f"Age/Sex: {_nz(prev.get('age_sex'),'Unknown')}\n"
        f"Present history: {_nz(prev.get('present_history'))}\n"
        f"Past history: {_nz(prev.get('past_history'))}\n"
        f"Subjective summary:\n{_nz(subj_summary)}\n"
        f"Prior assessments:\n{_nz(assess_summary)}\n\n"
        f"For the **{field.replace('_',' ').title()}** section, they selected: {_nz(choice)}.\n"
        "List 3–5 next assessment actions/tests (numbered only), including what positive/negative findings imply."
    )
    try:
...
# ---- AI: Provisional Diagnosis (generic; used on Objective screen button) ----
@app.route('/ai_suggestion/provisional_diagnosis', methods=['POST'])
@csrf.exempt
@login_required()
def ai_provisional_diagnosis():
    data = request.get_json() or {}
    age_sex = (data.get('age_sex') or '').strip()
    present = (data.get('present_history') or '').strip()
    past    = (data.get('past_history') or '').strip()

    prompt = (
        "Based on the following (no identifiers), give 3 likely provisional diagnoses with a 1-line rationale each.\n"
        f"Age/Sex: {_nz(age_sex,'Unknown')}\n"
        f"Present history: {_nz(present)}\n"
        f"Past history: {_nz(past)}\n"
        "Return a numbered list (3 items), no extra commentary."
    )
    try:
        suggestion = get_ai_suggestion(prompt).strip()
        return jsonify({'suggestion': suggestion})
...
def ai_smart_goals(field):
    ...
    prev = {
        ...
        "present_history": (data.get("previous", {}) or {}).get("present_history","").strip(),
        "past_history":    (data.get("previous", {}) or {}).get("past_history","").strip(),
    ...
    prompts = {
        'patient_goal':        "Draft one concise patient-centric SMART goal (numbered list with 1 item).",
        'baseline_status':     "State a clear baseline status for tracking the goal (1 bullet).",
        'measurable_outcome':  "List 2–3 measurable outcomes (numbered).",
        'time_duration':       "Suggest a realistic timeframe (1–2 bullets).",
    }
...
def treatment_plan_suggest(field):
    ...
    prompt = (
        "You are a physiotherapy assistant. Do not include identifiers.\n"
        f"Age/Sex: {_nz(prev.get('age_sex'),'Unknown')}\n"
        f"Present history: {_nz(prev.get('present_history'))}\n"
        f"Past history: {_nz(prev.get('past_history'))}\n"
        f"Assessments so far: {_nz(', '.join(prev.get('assessments', {}).keys()))}\n\n"
        f"Field: {field.replace('_',' ').title()}\n"
        f"Input: {_nz(text_input)}\n"
        "Suggest 3–5 specific, evidence-aligned interventions or parameters (numbered)."
    )
    try:
        suggestion = get_ai_suggestion(prompt).strip()
        return jsonify({'suggestion': suggestion})
...
def treatment_plan_summary(patient_id):
    pdoc = db.collection('patients').document(patient_id).get()
    patient = pdoc.to_dict() if pdoc.exists else {}

    prev = _collect_prev({}, patient)
    prompt = (
        "Create a concise treatment summary (no identifiers) that links goals to interventions and expected outcomes.\n"
        f"Age/Sex: {_nz(prev.get('age_sex'),'Unknown')}\n"
        f"Present history: {_nz(prev.get('present_history'))}\n"
        f"Key assessments: {_nz(', '.join(prev.get('assessments', {}).keys()))}\n"
        "Return 3–5 numbered points."
    )
    try:
...
def ai_followup_suggestion(patient_id):
    ...
    case_summary = "\n".join(filter(None, [
        ...
        f"Present history: {_nz(prev.get('present_history'))}",
        f"Past history: {_nz(prev.get('past_history'))}",
        f"Subjective: {_nz(', '.join([k for k,v in prev.get('subjective',{}).items() if v]))}",
        f"Assessments: {_nz(', '.join([k for k,v in prev.get('assessments',{}).items() if v]))}",
    ]))
    ...
    prompt = (
        ...
        "Suggest 2–3 focus points for the next session, aligned with prior findings and SMART goals."
    )
    try:
        suggestion = get_ai_suggestion(prompt).strip()
        return jsonify({'suggestion': suggestion})
